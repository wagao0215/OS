...
void test23(void) {
    ...
    GET_TIME_OF_DAY(&CurrentTime);
    printf("TEST 23:   Ends at Time %ld\n", CurrentTime);
    TERMINATE_PROCESS(-2, &ErrorReturned);
}           // End of test22
...
#define                 LOGICAL_PAGES_TO_TOUCH       4 * NUMBER_PHYSICAL_PAGES
...
#define           SLEEP_TIME_28                             10000

void test28(void) {
    long OurProcessID;     // PID of test28
    ...
    for (Iteration = 0; Iteration < 5; Iteration++) {
        sprintf(ProcessName, "Test28_%ld", Iteration);
        printf("Creating process \"%s\"\n", ProcessName);
...
Note:  the smaller the SKEWING_FACTOR, the more skewed the numbers are.
 November, 2016, this value was changed from 0.30 to 0.21.  This produces
 numbers that are on about 50+ pages when a range of 128 is requested.
 And thus all fit into physical memory.
 **************************************************************************/
/*
#define                 SKEWING_FACTOR          0.21
...
void GetSkewedRandomNumber(long *random_number, long range) {
    ...
    double multiplier = (double) extended_range / (double) MY_RAND_MAX;

    temp = multiplier * (double) (rand() % MY_RAND_MAX);
...
printf( "MY RAND_MAX Extended, temp, temp2, temp3 %6d %6ld  %6d  %6d  %6d\n",
       MY_RAND_MAX, extended_range, (int)temp, (int)temp2, (int)temp3 );
*/
void GetSkewedRandomNumber(long *ReturnedValue, long range) {
    int  RandomNumber;
    RandomNumber = (range * (rand() % 32767)) / 32767;
    RandomNumber = (int)(15 * pow(RandomNumber, 0.33));
